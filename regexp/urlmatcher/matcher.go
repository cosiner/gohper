// Package urlmatcher implements a matcher of url which convert an simple
// regexp syntax to standard golang regexp syntax and capture all variable
// values defined in regexp
package urlmatcher
...
const (
	// pattern start type
	_PROTO = iota // _PROTO means pattern is start with url protocal, such as http://
	...
	// url pattern type
	LITERAL        // LITERAL means a normal string without regexp
...
var (
	...
	// standCompiler is the standard regexp compiler
...
type (
	...
	Matcher interface {
		...
		IsLiteral() bool
		// Match match a url, and extract all url variable values if it's a
		// regexp matcher
...
// matchany character "."
func NewCompiler(regLeft, regRight byte, needEscape, replaceMatchany bool, matchany, noreplaceFlag string) *Compiler {
	if replaceMatchany && matchany == "" {
		matchany = _MATCHANY
	}
	if noreplaceFlag == "" {
...
func NewStandardMatcher(patternType int, pattern string) (
	Matcher, error) {
	if pattern == "" {
...
func (c *Compiler) Compile(urlPattern string) (int, string) {
	if urlPattern == "" {
		return ERROR_FORMAT, ""
	}
	var (
...
func writeRegexp(buf *bytes.Buffer, replace bool, group, regexp, replaceMatchany []byte) (success, hasGroup bool) {
...
// patternStart check with section is the url start with: PROTO or HOST or PATH
func patternStart(urlPattern string) (start int) {
	first := urlPattern[0]
	switch {
